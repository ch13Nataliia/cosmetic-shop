...
export default function BlogPost({ ssd = {} }) {
  ...
  return (
    <>
      ...
      <Layout>
        <Heading component="h2" variant="h4">Single Post</Heading>
...
export const getStaticPaths = async () => {
  const allPosts = await fetch(process.env.HYGRAPH_ENDPOINT, {
    ...
    .then((res) => res.json())
    .then((res) => {
      console.log(res.data.sevices);
      return res.data.sevices;
    })
...
export async function getStaticProps({ params }) {
  // console.log(params);
  const { sevice: post } = await fetch(process.env.HYGRAPH_ENDPOINT, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${process.env.HYGRAPH_TOKEN}`,
